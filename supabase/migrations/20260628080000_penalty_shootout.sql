-- ============================================================
-- Migration: Penalty Shootout Support
-- Adds penalty_winner to match_results and predictions tables,
-- marks matches as knockout, and updates scoring to award 6 pts
-- for a correct penalty winner prediction.
-- ============================================================

-- 1. Add penalty_winner to match_results table
...
-- 2. Add predicted_penalty_winner to predictions table
ALTER TABLE public.predictions
  ADD COLUMN IF NOT EXISTS predicted_penalty_winner TEXT CHECK (predicted_penalty_winner IN ('home', 'away', NULL));

-- 3. Add penalty_correct_picks to leaderboard table
ALTER TABLE public.leaderboard
  ADD COLUMN IF NOT EXISTS penalty_correct_picks INTEGER NOT NULL DEFAULT 0;

-- 4. Update calculate_match_points to handle penalty shootout scoring
-- Points awarded:
--   Exact 90-min score:           +3 pts (unchanged)
--   Correct result (win/draw):     +1 pt  (unchanged)
--   Correct penalty winner pick:   +6 pts (NEW — only applies when is_knockout AND actual score is draw)
--   Wrong prediction:               0 pts  (unchanged)
...
BEGIN
  ...
  -- Determine if 90-min score is a draw
  ...
  FOR v_pred IN SELECT * FROM public.predictions WHERE match_id = p_match_id LOOP
    ...
    -- Penalty winner check: only applies for knockout matches where the 90-min score is a draw
    ...
    -- Calculate points
    IF v_is_exact THEN
      -- Exact 90-min score gets base 3 pts + optional 6 pts for penalty winner
      v_points := v_settings.exact_score_points;
      IF v_is_penalty_correct THEN
        v_points := v_points + 6;
      END IF;
    ELSIF p_is_knockout AND v_is_draw AND p_penalty_winner IS NOT NULL
          AND v_pred.predicted_home_score = v_pred.predicted_away_score THEN
      -- Predicted a draw in a knockout match (correct that it's going to penalties)
      -- +1 for correct result (draw after 90 mins) + optionally +6 for penalty winner
      v_points := v_settings.correct_result_points;
      IF v_is_penalty_correct THEN
        v_points := v_points + 6;
      END IF;
    ELSIF v_is_correct THEN
      -- Correct non-draw result for knockout, or any correct result in group stage
      v_points := v_settings.correct_result_points;
    ELSE
...
-- 5. Update the trigger function to pass the new columns
